...
function Listbox() {
  ...
  return (
    <>
      <ComboBox options={options} value={fruit} onChange={handleFruitChange} isListBox>
        <ComboBoxInput
          placeholder="Select fruit..."
          renderSelectedOption={option => (
            <Flex w="100%" alignItems="center" cursor="default">
...
function ComboBoxExample() {
  ...
  return (
    <>
      <ComboBox options={options} value={fruit} onChange={handleFruitChange}>
        <ComboBoxRightElement />
        <ComboBoxClearElement />
...
function ComboBoxExample() {
  const getOptionStyles = (selected, highlighted, disabled) => {
    ...
    const styles = {
      ...
      px: 4,
      bg: selected ? `teal.300` : highlighted ? `teal.100` : "transparent",
      _active: {
        bg: "gray.200",
      },
      _focus: {
...
## ComboBox with Customizable option select
